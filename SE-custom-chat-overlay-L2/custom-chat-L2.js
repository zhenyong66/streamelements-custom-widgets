...
window.addEventListener('onEventReceived', function (obj) {
  ...
  /*let userIdentity;
  if (obj.detail.event.data.tags.mod == 1) {
    userIdentity = "mod;
  }*/
    addMessage(username, badges, message, data.isAction, data.userId, data.msgId, data.tags.mod);
});
...
function addMessage(username, badges, message, isAction, uid, msgId, isMod) {
    totalMessages += 1;
    let actionClass = "";
  let userBoxColor = "", userMsgColor="";
  /*if (isMod) {
    userBoxColor = "modBoxColor";
    userMsgColor = "modMsgColor";
  }*/
    if (isAction) {
        actionClass = "action";
    }
    ...
    <div data-sender="${uid}" data-msgid="${msgId}" class="message-row {animationIn} animated" id="msg-${totalMessages}">
        <div class="user-box ${actionClass} ${userBoxColor}">${badges}${username}</div>
        <div class="user-message ${actionClass} ${userMsgColor}">${message}</div>
    </div>`);
    if (addition === "append") {
        if (hideAfter !== 999) {
